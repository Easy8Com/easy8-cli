...
If auth/config is missing, stop and ask for env setup with one focused message.
...
```bash
go run ./cmd/easy8 issue search --q "<user text>" --json
```
...
```bash
go run ./cmd/easy8 pbi list --q "<user text>" --json
```
...
- Always prefer `--json` for machine parsing.
